...
export default class Async extends React.PureComponent {
    static propTypes = {
        ...
        /** a promise, a function that returns a promise, or some other piece of data to be run through `props.convertToJSXFunc` */
        data: PropTypes.any,

        /** content to be shown if the promise is rejected; if providing a promise generated by the fetch API, a "not ok" response is considered an error */
    ...
    convertDataToJSXOrWait(props = this.props) {
        ...
        if (typeof data === 'function') {
            ...
            if (!(data instanceof Promise)) {
                throw new Error('Boundless.Async: called props.data function and it did not return a Promise.');
            }
        }
        ...
        if (data instanceof Promise) {
            ...
            data.then((payload) => {
                if (payload instanceof Response && payload.ok === false) {
                    return this.setState({component: false});
                } // the fetch API doesn't reject promises on non-fatal errors, so we'll handle that
